 ...
 * 
 * this plugin restore the HUD feature as implemented in version <= 0.9.8
 * and also restore the API entry points in `me.game` (`addHUD` and `disableHUD`)
 * usage : `me.plugin.register(HUD, "hud");`
 * 
 * note : this has been superseed by the usage of `me.entityObject`
 */
...
(function($) {
	...
	/** @scope me.debug.Panel.prototype */
	{
		...
		init : function() {
			 ...
			 */
			me.game.addHUD = function(x, y, w, h, bg) {
				// if no HUD existing
				if (me.game.HUD == null) {
					// create a new default HUD object
					me.game.HUD = new me.HUD_Object(x, y, w, h, bg);
